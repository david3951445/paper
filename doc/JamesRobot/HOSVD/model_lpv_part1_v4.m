clc;clear;close all
load('model_para_part1_v4.mat');
addpath 'G:\我的雲端硬碟\畢業論文 進度\模擬\MATLAB\biped robot'
% for x(1) ~ x(4)
%% lpv
lpv = {...
    @(x)Jt1_1(x)     @(x)Jt2_1(x)      @(x)Jt3_1(x)     @(x)Jt4_1(x) ; 
    @(x)Jt1_2(x)     @(x)Jt2_2(x)      @(x)Jt3_2(x)     @(x)Jt4_2(x) 
    };
...
save('lpv_data_part1_v4', 'lpvdata', 'S', 'U', 'domain', 'gridsize');
