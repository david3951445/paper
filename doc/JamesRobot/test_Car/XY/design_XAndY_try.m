    ...
    for j = 1:size(LP1.S,2)
        for k = 1:size(LP1.S,3)
            Abar(:,:,c) = [...
                ...
                A1{i,j,k}*S6 - S1*Ar       zeros(2,2)  A1{i,j,k}    zeros(2,1)     ;
...
% QbarS = sqrtm(Qbar);
Rbar =  1e-4 * (eye(2) + epsilon^2*eye(2));
invRbar = inv(Rbar);
go = [zeros(10,2);eye(2)];
lo = 1;
dF = 0.001;
...
%     Y = sdpvar(2,18,'full');
    ...
    P11 = Abar(:,:,i)*W + W*Abar(:,:,i)' + go*Y(:,:,i) + Y(:,:,i)'*go' + 2*eye(12) + (1/lo)^2*D2;
    P21 = Y(:,:,i);
    P22 = -invRbar;
    P31 = Qbar*W;
    P32 = zeros(12,2);
