function uav = trajectory(uav, ref, fz)
...
for i = 1 : LEN - 1       
    ...
    k1 = RK4(uav, fz, ref, xb(:, i), i*dt);
    if uav.tr.IS_RK4
        k2 = RK4(uav, fz, ref, xb(:, i)+k1*dt/2, i*dt + dt/2);
        k3 = RK4(uav, fz, ref, xb(:, i)+k2*dt/2, i*dt + dt/2);
        k4 = RK4(uav, fz, ref, xb(:, i)+k3*dt, i*dt + dt); 
...
%% Local function
function k = RK4(uav, fz, ref, xb, t)
    O       = zeros(uav.DIM_X);
