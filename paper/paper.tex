...
Through above analysis, the tracking control problem of the nonlinear system with external disturbance (\ref{eq:agent d1}) is transformed into the fault tolerance stable control problem of linear system (\ref{eq:linear f1}). The remaining step is to design an appropriate feedback control law $u_{fb}(t)$ to make it stable. In a real system, the feedback information is measured by sensor, i.e, the state $\pmb{e}(t)$ in (\ref{eq:linear f1}) is unavailable. As mentioned before, the external disturbances on sensor also need to be considered. Since the sensor information (\ref{eq:agent output}) will transmitted back to the ground station through the network channel in URTS, not only the sensor noise but also the coupling effect due to co-channel interference and the cyber-attack signal are considered. The total effect can be equivalent to an sensor fault signal $f_2(t)$. The measurement output equation is formulated as:
\begin{equation} \label{eq:agent output}
    y(t) = C\pmb{e}(t) + f_2(t)
\end{equation}
where $y(t)\in\mathbb{R}^l$ is output vector, $C\in\mathbb{R}^{l\times 3n}$ is output matrix. By combining (\ref{eq:linear f1}) and (\ref{eq:agent output}) together, we have:
