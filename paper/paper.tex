...
Through above analysis, the tracking control problem of the nonlinear system with external disturbance (\ref{eq:agent d1}) is transformed into the stable control problem of the linear system (\ref{eq:linear f1}) with fault signal by the feedforward linearization. The remaining step is to design an appropriate feedback control law $u_{fb}(t)$ to make the linear system stable. In a real system, the feedback information is measured by sensor, i.e, the state $\pmb{e}(t)$ in (\ref{eq:linear f1}) is unavailable. At the same time, the external disturbances on sensor also need to be considered as mentioned before. Since the sensor information will transmitted back to the ground station for calculating control command through the network channel in URTS, not only the sensor noise but also the coupling effect due to co-channel interference and the cyber-attack signal are concerned. Suppose the total effect can be equivalent to an unknown sensor fault signal $f_2(t)\in \mathbb{R}^{n}$ with same size as $f_1(t)$, the measurement output equation can be described as:
\begin{equation} \label{eq:agent output}
    y(t) = C\pmb{e}(t) + B_2f_2(t)
\end{equation}
where $y(t)\in\mathbb{R}^{ln}$ is the output vector, $C\in\mathbb{R}^{l\times 3n}$ is the output matrix, and $B_2\in\mathbb{R}^{l\times n}$ is the input matrix of $f_2(t)$. By putting (\ref{eq:linear f1}) and (\ref{eq:agent output}) together, we have the following system:
...
\end{bmatrix}^T$, and $A_i=[a_{j,k}]\otimes I_{n}$. Then, the fault signals $f_i(t), i=1,2$ are the output of the system in (\ref{eq:smooth model})
\begin{equation} \label{eq:8}
    f_i(t)=C_iF_i(t)
\end{equation}
where $C_i=\begin{bmatrix}
    1 & 0 & \dots & 0
\end{bmatrix}\otimes I_{n}$. Sub. (\ref{eq:smooth model}) and (\ref{eq:8}) into (\ref{eq:error}), we get:
...
\begin{equation} \label{Hinf}
    ...
    \end{split}
\end{equation}
where $t_f$ is the final time, $Q \geq 0$ is the weighting matrix of state and estimation error, $R > 0$ is the weighting matrix of control effort, $V(\tilde{x}_0)$ is the initial condition effect on augmented system(\ref{eq:aug}), and $\tilde{v}(t)$ is the total disturbance needs to be attenuated. If we can find the control gain $K$ and observer gain $L$ such that (\ref{Hinf}) holds, then the effect of total disturbance $\tilde{v}(t)$ on tracking and estimation error $\tilde{x}(t)$ can be attenuated to a prescribed level $\rho^2$ from the viewpoint of energy. Before analyzing the $H_\infty$ observer-based stablized control problem in (\ref{Hinf}), the following lemmas are given:
...
%     \end{equation}
% \end{proof}
Although the control gain and observer gain can already be found through the previous steps, the calculation speed can be further improved by reducing the dimensionality of the LMI (\ref{BMI1}). First, let us decompose the PID tracking error $\pmb{e}(t)=\sum_{i=1}^{n} \pmb{e}_i(t)\otimes\mathbf{e}_i$ and the output $y(t)=\sum_{i=1}^{n} {y}_i(t)\otimes\mathbf{e}_i$ where $\pmb{e}_i(t)\in\mathbb{R}^3$, $y_i(t)\in\mathbb{R}^l$ and $\mathbf{e}_i$ is standard unit column vectors in $\mathbb{R}^n$. Observing the matrices $A,B,C,B_2$ in the linearized system (\ref{eq:error}), it can be further split into $n$ subsystems if the matrices $C,B_2$ in the output equation (\ref{eq:agent output}) have the same form to $A,B$, i.e., $C = C_0\otimes I_n,B_2 = B_{2,0}\otimes I_n$ where $C_0\in\mathbb{R}^{l\times 3},B_{2,0}\in\mathbb{R}^{l\times 1}$. This means that $\pmb{e}_i(t)$, the PID error of each state variable $x(t)$ of agent in (\ref{eq:agent}), can be measured independently via sensors to obtain the independent outputs $y_i(t)$. In actual systems, this is usually done. Due to the fact that $f_1(t)\in\mathbb{R}^n, f_2(t)\in\mathbb{R}^{n}$, the n subsystems are obtained from (\ref{eq:error}):
\begin{equation} \label{eq:linear subsys}
    ...
    \end{split}
\end{equation}
where $u_{fb,i}(t),f_{1,i}(t),f_{2,i}(t)\in \mathbb{R}$ denote the element in the corresponding vector. By Theorem \ref{theorem1} again, the form shows that the subsystems (\ref{eq:linear subsys}) can achieve the $H_\infty$ observer-based stablized control performance. Let $K_i$ be the control gain of the $i$th subsystems, the origin control gain $K$ of the origin agent system can be obtained by $K = \begin{bmatrix}
    \mathit{v}_1 & \mathit{v}_2 & ... & \mathit{v}_n
\end{bmatrix}^T, \mathit{v}_i=K_i^T\otimes\mathbf{e}_i$. The origin observer gain $L$ can be obtained in the same way. In this case, the calculate speed can be improved since the dimensionality is decrease.
\textit{
    \begin{remark}
