...
\textit{
    \begin{remark}
        Since we set $C = C_0\otimes I_n$ and $B_2 = B_{2,0}\otimes I_n$, the proposed method of decomposing each agent model into $n$ subsystems is used to find $K$ and $L$. For the convenience of solving, we set the same design parameters $Q_1$, $Q_2$, $R$ and $\rho^*$ for each subsystem as shown above.
    \end{remark}
}
The simulation results of tracking and estimation in tracking control block of the UAV $\alpha_{1,1}$ and the robot $\alpha_{1,5}$ in $team_1$ are given as follows:
...
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=.57]{fig/robot (1) noFTC.png}\caption{The trajectories of reference, state and estimated state of the robot $\alpha_{1,5}$ by traditional PID computed torque controller without FTC \cite{7456706}}%
