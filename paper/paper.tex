...
In Fig. \ref{fig:LocalMotionPlanning}, some basic behaviors of UAV and robot mentioned before are given to futher illustrate the flow of this block. To limit the scope of this article, we only focus on the motion planning of flying behavior of UAV and walking behavior of robot. They are belong to the moving behavior in Fig. \ref{fig:LocalMotionPlanning}. A more detailed description will be given in the next section.
...
\textit{
    \begin{remark}
        Since the quadrotor has four rotors, we only have four control input. To simplify the model, the UAV (quadrotor) dynamic model in (\ref{eq:uav}) considers $u'(t)=[F, \tau_u^T]^T=[F, \tau_x, \tau_y, \tau_z]^T$ as the equivalent control input for the four rotors. Besides, since $u'(t)\in\mathbb{R}^4$ has two less degrees of freedom than $r(t)\in\mathbb{R}^6$, UAV is an underactuated system.
    \end{remark}
}
...
\begin{figure*}[htbp]
    \centering
    \includegraphics[scale=.12]{fig/tracking.png}\caption{The flowchart of tracking control in the hybrid URTS. (a) The tracking control block of each agent in hybrid URTS. The reference generator block and control block are predefined processes defined in (b) and (c). The control block (the proposed general $H_\infty$ decentralized observer-based feedforward FTC scheme) is designed for a fully actuated agent dynamic model in (\ref{eq:agent}) while the UAV is an underactuated system. It makes the designed control law $u(t)\in\mathbb{R}^6$ and the actual control input $u'(t)\in\mathbb{R}^4$ different for UAV. The reference generator block is introduced to deal with this problem. (b) The reference generator blocks for UAV and robot are different since UAV is an underactuated system. For UAV, we need to calculate $u'(t)$ and $r(t)$ from $u(t)$ and $r'(t)$ by inverse dynamic through the UAV dynamic model in (\ref{eq:uav}). For robot, $u'(t)=u(t)$ and $r(t)=r'(t)$ since the system is fully actuated. (c) The proposed general $H_\infty$ decentralized observer-based feedforward FTC scheme for each agent in hybrid URTS.}
    \label{fig:tracking}
\end{figure*}
...
\end{assumption}
The robust FTC law of each agent is given as:
...
The design procedure of the optimal $H_\infty$ decentralized observer-based feedforward FTC scheme for each agent in (\ref{eq:agent d1}) is summarized as follows:
\begin{enumerate}
    ...
    \item Construct the robust FTC law (\ref{eq:u_fb}) and the augmented estimation error system (\ref{eq:e_tilde}) to obtain the augmented tracking and estimation error system in (\ref{eq:x_tilde})
