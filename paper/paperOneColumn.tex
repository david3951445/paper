...
\textit{
    \begin{remark}
        In this paper, we will only state and solve the problems in Local Motion Planning and Reference Tracking Control, i.e., how to plan the local motion of UAV flying and biped robot walking and how to design the robust $H_\infty$ decentralized observer-based feedforward reference tracking FTC strategy for each agent in URTS. Nevertheless, the reason why we still state the problems in Task Allocation, Global Path Planning and Behavior Decision is to clarify the systematic relationship between these blocks, which provides a blueprint for those who want to construct a system like URTS.
    \end{remark}
}
...
From above assumptions, a global path of agent can be expressed as a sequence:
\begin{equation}
    (\sigma_n), n\in\mathbb{Z}\cap[1,k_f], \sigma_n\in\mathcal{C}
\end{equation}
where $k_f$ is the time step when reaching goal. $(\sigma_n)$ then passes to next block, i.e., Behavior Decision block, as shown in Fig. \ref{fig:sys}.
...
\begin{figure*}[htbp]
    ...
    \label{fig:FandW}
\end{figure*}
Before the discussion of the local motion planning of these two behaviors of UAV and robot in URTS, the following assumption is maded.
\textit{
...
$g$ is the gravity acceleration, $m$ and $J$ are the mass and inertia matrix of UAV, respectively, $\tau_u$ and $F$ are the total torque and force acting on UAV, respectively, $\Theta$ is the Euler angles in body frame as shown in Fig. 2.8 in \cite{sabatino2015quadrotor} with $\phi\in[ -\pi, \pi ]$, $\theta\in[ -\pi/2, \pi/2 ]$ and $\psi\in[ -\pi, \pi ]$, $X$ is the postion of center of mass (CoM) in inertial frame, $K_\tau$ and $K_F$ are the aerodynamic damping coefficients, and $R(\Theta)$ is the intrinsic rotation matrix from body frame to inertial frame. This model treats the UAV as a mass point and can control the total force $F$ and the total torque $\tau_u$. For UAV, the reference trajectory $r(t)=[x_r(t),y_r(t),z_r(t),\phi_r(t),\theta_r(t),\psi_r(t)]^T\in\mathbb{R}^{6}$ is in the task space, where the subscript $r$ denotes the reference.
...
The decentralized feedforward linearization control law for an agent in URTS is proposed as:
...
\textit{
    \begin{remark}
        The solution of $\phi_r$ and $\theta_r$ must satisfy the constraints $\phi_r\in[-\pi,\pi]$ and $\theta_r\in[-\pi/2,\pi/2]$ in UAV dynamic model in (\ref{eq:uav}). In addition, since $\phi_r$ and $\theta_r$ are calculated at each time step and there may be multiple solutions due to inverse dynamic calculation, the solution of $\phi_r$ and $\theta_r$ must be selected to be continuous with the previous time step.
    \end{remark}
}
...
\textit{
    \begin{remark}
        Under the decentralized control architecture, the coupling effect $c(t)$ between agents becomes their own external disturbance, which greatly reduces the difficulty of controller design.
    \end{remark}
}
...
%     \end{remark}
% }
Since the fault signals become a state variable of the augmented tracking error system of an agent in (\ref{eq:e_bar}), their corruption on the tracking error dynamic system in (\ref{eq:error}) can be avoided. In addition, the amplitude of the fault signals $f_i(t),i=1,2$ no longer affect the augmented tracking error system in (\ref{eq:e_bar}), but the smoothness of $f_i(t),i=1,2$, i.e., the amplitude of the smoothing signal model error $\bar{v}(t)$. This allows the augmented tracking error system in (\ref{eq:e_bar}) to tolerate the fault signals with larger amplitudes. A Luenberger observer is proposed to estimate them and $\pmb{e}(t)$ simultaneously to achieve an active FTC by the following estimation system:
